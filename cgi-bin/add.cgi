...
if(! (-e $sharedconf)){
    ...
    foreach my $component (sort keys %$componentconf){
        my $outputfile = "$outputdir/$component/${pipeline_id}_TOKEN/component.conf.bld.ini";
        print "<a target='config' href='config_component.cgi?conffile=$componentconf->{$component}&sharedconf=$sharedconf&ignoresect=init&pipeline_id=$pipeline_id&component_name=$component'>$component</a><br>";
    }
}
