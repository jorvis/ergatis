...
my $builder = new Ergatis::Builder('NAME'=>'blastp', #verbose debugging
				    'REPOSITORY_ROOT'=>'/usr/local/annotation',
				    'DATABASE'=>'tryp'
				    );

B<Returns:> $self (A Ergatis::Builder object).
...
sub new {
    ...
    $self->{_logger} = Ergatis::Logger::get_logger(__PACKAGE__);
    $self->_init(@_);
    return $self;
}
...
sub _init {
    ...
    $self->{_WORKFLOW_ROOT} = '/usr/local/devel/ANNOTATION/workflow-3.beta/';
    $self->{_nodistrib} = 0;
