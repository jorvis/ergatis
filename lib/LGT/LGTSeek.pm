...
sub prinseqFilterBam {
    ...
    my $retval;
    if ( $self->{paired_end} ) {
        $retval = $self->_prinseqFilterPaired( $config->{input_bam}, $config->{output_dir}, $config->{tmp_dir}, $overwrite );
    }
    else {
    ...
    if ( -e "$output_dir/$name\_bad_ids.out" && $overwrite == 0 ) {
        ...
        if ( $rm_low_cmplx == 1 ) {
            ...
            if ( -e "$tmp_dir/$name\_lc_1_bad.fastq" ) {
                ...
                $self->_run_cmd("/usr/bin/perl -e 'while(<>){s/\\@//;s/\\/\\d//;print;<>;<>;<>;}' $tmp_dir/$name\_lc_1_bad.fastq > $tmp_dir/$name\_lc_1_bad_ids.out");
            }
            else {
            ...
            # Run prinseq for low complexity filtering
            $self->_run_cmd("/usr/bin/perl $prinseq_bin --fastq=$tmp_dir/$name\_2.fastq --out_good null --out_bad=$tmp_dir/$name\_lc_2_bad -lc_method dust -lc_threshold 7");

            # Pull out bad ids
            if ( -e "$tmp_dir/$name\_lc_2_bad.fastq" ) {
                if ( $self->{verbose} ) { print STDERR "========= Pull Low-Cmplx-2 Bad ID's ========\n"; }
                $self->_run_cmd("/usr/bin/perl -e 'while(<>){s/\\@//;s/\\/\\d//;print;<>;<>;<>;}' $tmp_dir/$name\_lc_2_bad.fastq > $tmp_dir/$name\_lc_2_bad_ids.out");
            }
            else {
...
sub sam2Fasta {
    ...
    my $cmd = "/usr/bin/perl $bin/sam2fasta.pl --samtools_bin=$self->{samtools_bin} --input=$config->{input}";
    if ( $config->{fastq} ) {
