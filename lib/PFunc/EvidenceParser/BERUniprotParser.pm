...
my $default_characterized_info = "/usr/local/projects/db/tchar/tchar.db";
my $default_uniprot_info = "/usr/local/scratch/kgalens/uniprot/uniprot_info";
...
sub _init_ber_parser {
    ...
    ## uniprot lookup file
    my $database = ( $args{'uniprot_lookup'} ) ? $args{'uniprot_lookup'} : $default_uniprot_info;
    $self->{'_uniprot_lookup'} = new PFunc::UniprotLookup( 'database' => $database );

    ## characterized info
    my %tmp;
    if( $args{'characterized_info'} ) {
        tie(%tmp, 'MLDBM', $args{'characterized_info'}, O_RDONLY )
            or die("Could not tie hash to $args{'characterized_info'}");
    } else {
        tie(%tmp, 'MLDBM', $default_characterized_info, O_RDONLY )
            or die("Could not tie hash to $default_characterized_info");
    }
    $self->{'_characterized_info'} = \%tmp;
...
sub _characterized_info {
    my ($self, $id) = @_;
    my $retval;
    if( !defined( $id ) ) {
        $retval = $self->{'_characterized_info'};
    } elsif( exists( $self->{'_characterized_info'}->{$id} ) ) {
        $retval = $self->{'_characterized_info'}->{$id};
    }
    return $retval;
}
...
sub _uniprot_info {
    ...
    my $retval;
    if( !defined( $id ) ) {
        $retval = $self->{"_uniprot_lookup"};
    } elsif( exists( $self->{'_uniprot_lookup'}->{$id} ) ) {
        $retval = $self->{'_uniprot_lookup'}->get_info( $id );
    }
    return $retval;
}
...
sub _handle_seq_pair_alignment {
    ...
    my $confidence_level = $self->_assign_confidence_level( $query_coverage, $subject_coverage, 
                                                            $self->_characterized_info( $self->_formatted_seq_id( $comp_id ) ) );
...
sub _get_compseq_annotation {
    ...
    #grab information from the uniprot lookup
    my @info = $self->_lookup_uniprot_info( $formatted_id );

    # make the annotation object
    my $source = $formatted_id;
    ...
    foreach my $field ( qw(gene_product_name EC gene_symbol TIGR_Role GO) ) {
        $ret_annot->set( $field, shift @info, $source, $confidence_level );
    }

    return ($ret_annot, pop @info );
    
}

sub _lookup_uniprot_info {
    my ($self, $id) = @_;
    
    my ($com_name, $ec, $gene_symbol, $role, $go, $is_char);

    my $protein_info = $self->_uniprot_info( $id );
    print Dumper( $protein_info );
    exit(0);
    die("Could not find protein info for $id") unless( defined( $protein_info ) );
    $com_name = $protein_info->{'com_name'};
    $ec = [$protein_info->{'ec'}] if( defined( $protein_info->{'ec'} ) );
    $gene_symbol = $protein_info->{'gene_symbol'} if( defined( $protein_info->{'gene_symbol'} ) );
    $is_char = 0;
    if( my $info = $self->_characterized_info( $id ) ) {
        $is_char = 1;
    } elsif( $protein_info->{'protein_existence'} == 1) {
        $is_char = 1;
    }

    return ($com_name, $ec, $gene_symbol, $role, $go, $is_char);
}
...
sub _parse_protein_header_line {
    ...
    if( $full_id ) {
        ...
        my $info = $self->_characterized_info( $look_id );
