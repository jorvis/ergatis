...
sub _handle_sequence {
    ...
    my ($defline_att) = $seq->find_nodes('Attribute[@name="primary_label"]');
...
sub _handle_seq_pair_alignment {
    ...
    print STDERR "Could not get annotation_feature_id\n" unless( $annotation_feature_id );
    ...
    print STDERR "Confidence level: $confidence_level\n";
    ...
    print STDERR "Did not pass cutoffs\n" unless( &_match_passes_cutoff( $spa ) );
    ...
    if( $self->_is_name_ambiguous( $gp_name ) && !$is_char ) {
        print STDERR "Assigning as conserved hypo\n";
        $self->_assign_as_conserved_hypothetical( $annotation, $comp_annot->get_feature_id );
    } else {
        # if the current protein hasn't been annotated yet, automatically assign the current annotation
        if( !($annotation->has_annotation()) ) {
            print STDERR "Assigning current annotation\n";
            ...
            if( $ber_annot_levels->{ $comp_annot->_get_type( 'gene_product_name' ) } <
                ...
                if( $comp_count > $cur_count ) {
                    print STDERR "assigning other annotation\n";
                    $self->_assign_annotation( $annotation, $comp_annot );
                }
            }
        }
...
sub _match_passes_cutoff {
    ...
    my ($attribute) = $spa->find_nodes('Attribute[@name="percent_identity"]');
    if( $attribute ) {
...
sub _calculate_spr_coverage {
    ...
    foreach my $spr ( @sprs ) {
        $query_hit_length += $spr->att('runlength');
        $subject_hit_length += $spr->att('comprunlength');
    }
