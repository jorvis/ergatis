...
nlmodel_exp <- nls(v2means ~ th1 + th2* exp(-v1means / th3), data=pangenome,
start=list(th1=33, th2=476, th3=1.5))
#summary(nlmodel_exp)

# Open up the output file for the graph
postscript(file="###output_path###core_genes_exponential_means_error_bar_with_points.ps", width=11, height=8.5, paper='special')
layout(matrix(c(1,2),byrow=TRUE), heights=c(7.5,1))

# Draw the axis
plot(V1,V2, xlab="number of genomes", ylab="core genes", main="###TITLE### core genes exponential axis", cex=0.2, col=p_color)
