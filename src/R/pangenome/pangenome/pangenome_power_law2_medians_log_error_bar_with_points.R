...
nlmodel_pot <- nls(v2submedians ~ th1*v1submedians^(th2), data=pangenome,
start=list(th1=2000, th2=0))
#summary(nlmodel_pot)

# Open up the output file for the log graph
postscript(file=paste(sep="", "###output_path###pangenome_power_law_medians_log_min_",gcnt,"_error_bar_with_points.ps"), width=11, height=8.5, paper='special')
...
lines(x, predict(nlmodel_pot, data.frame(v1submedians=x)), lwd=2, col="red")

y <-predict(nlmodel_pot, 0)
# plot the regression
#abline(lm(log10(v1submdians)~log10(v1submedians))) 
