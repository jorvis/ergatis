...
my $results = GetOptions(
    \%options,           'cogFile|c=s',
    'bsmlModelList|m=s', 'outputDir|o=s',
    'outputToken=s',     'maxCogSeqCount|s=s',
    'extension|e=s',     'use_feature_ids_in_fasta=i',
    'map_file|p=s',      'transformDir|t=s',
    'log|l=s',           'debug=s'
);
...
my $use_feature_ids_in_fasta = defined( $options{'use_feature_ids_in_fasta'} )
  && ( $options{'use_feature_ids_in_fasta'} > 0 );
...
my %seqParserArgs =
  ( ReadFeatureTables => 0, SequenceCallBack => \&createPolypeptideLookup );
...
# If transformed directory is not present, use the output directory
if ( !$transformDir ) {
    $transformed = $outDir;
} else {
    if ( !-d $transformDir ) {
...
while ( my $bsmlFile = <FILE> ) {
    ...
    if ( -e $bsmlFile ) {
        my $ifh;
        if ( $bsmlFile =~ /\.(gz|gzip)$/ ) {
            open( $ifh, "<:gzip", $bsmlFile )
              || die "can't read input file $bsmlFile: $!";
        } else {
            open( $ifh, "<$bsmlFile" )
              || die "can't read input file $bsmlFile: $!";
        }
...
sub outputCog {
    ...
    if ( scalar( @{$list} ) > 1 ) {
        ...
        if ( @{$list} <= $maxCogSeqCount ) {
            open( OUTFILE,
                ">$outDir/$cog.$options{'outputToken'}.$options{'extension'}" )
              or $logger->logdie("could not open $cog.$options{'extension'}");
            if ( defined $mapFile ) {
                open( FW,
                    ">$transformed/$cog.$options{'outputToken'}.$options{'extension'}"
                  )
                  or
                  $logger->logdie("could not open $cog.$options{'extension'}");
            }
            foreach my $seq ( @{$list} ) {
                print OUTFILE ">$seq\n";
                if ( defined $mapFile ) {
                    if ( exists( $hMap{$seq} ) ) {
                        ...
                        print FW ">$seq\n";
                        print STDOUT
                          "Could not find mapping for $seq in $mapFile file\n"
                          if defined $mapFile;
                    }
                }
                my $residues = undef;
...
sub createPolypeptideLookup {
    ...
    if (   ( $seqRef->returnattr('molecule') eq 'aa' )
        || ( $seqRef->returnattr('class') eq 'polypeptide' ) )
    {
