...
augustus2legacydb.pl - load augustus BSML file into the legacy database schema

=head1 SYNOPSIS

USAGE:  augustus2legacydb.pl
...
B<--bsml_file,-b> 
    Path to a single BSML augustus result file.

B<--list_file,-i> 
    Path to a list of augustus BSML output files.

B<--database,-d> 
    Sybase database name for the project in which these augustus predictions
    will be loaded.
...
This script is used to load gene predictions from the augustus program, in BSML format,
into one of the legacy databases.  Such BSML files are most likely generated from
the augustus workflow component.

=head1 INPUT

The input is a BSML result file.  See the augustus component documentation for the
...
be in the format like:
    ...
    aa1.assembly.16296.0.augustus.bsml
...
for (@files) {
    ...
    ##  aa1.assembly.(15588).0.augustus.bsml
    my $asmbl_id;
    if ($file =~ /\.assembly\.(\d+)/) {
        $asmbl_id = $1;
    } else {
    ...
    my $loader = new Annot_prediction_loader($dbh, $asmbl_id, 'augustus');
