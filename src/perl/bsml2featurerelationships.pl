        ...
        if ( -f $list && -s $list ) {
            ...
            for (<$fh>) {
                ...
                if ( -e $_ && -s $_ ) {
                    ...
                    $logger->warn("Error reading $_ from list $list")
                      if ( $logger->is_warn );
                }
            }
        } else {
  ...
  )
{
    ...
    if ( -d $thing ) {
        opendir( my $dh, $thing )
          || $logger->warn("Unable to access $thing due to $!");
...
open OUTFILE, ">$options{'output'}"
  or $logger->logdie("Can't open output file $options{'output'}");
...
foreach my $group (@groups) {
    ...
    foreach my $type (@srted_types) {
        ...
        map {
            if ( $coords{$_} ) {
				# First feature type from 'output_order' gets copy of coordinates
    ...
    my @vals = keys %{ $group->{ $srted_types[0] } };
...
sub add_file {
    ...
    if ( -e $file && -s $file ) {
        $logger->debug("Adding file $file for processing")
          if ( $logger->is_debug );
        push @files, $file;
    } else {
...
sub check_parameters {
    ...
    unless ( $options{bsml_input}
        || $options{bsml_list}
        || $options{bsml_file}
        || $options{bsml_dir} )
    {
        $logger->logdie(
            "You must specify input with --bsml_input --bsml_list --bsml_file or --bsml_dir"
        );
    }
    ...
    unless ( $options{output} ) {
        $logger->logdie(
            "You must specify an output directory or file with --output");
    }
