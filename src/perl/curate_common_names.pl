...
my $results = GetOptions(
    \%options,       'username|u=s',
    'password|p=s',  'password_file|P=s',
    'host|s=s',      'input_map_file|i=s',
    'no_change|n=s', "log|l=s",
    "debug|D=s",     "help|h"
);
...
if ( -e $rules_file ) {
    open( RF, "< $rules_file" )
      or die "Could not open $rules_file file for reading\n";
    my @contents = <RF>;
    close(RF);
...
$dbh =
  &_connect( $options{'username'}, $password, $database, $options{'host'} );
...
sub load_changes {
    ...
    my $query =
      "UPDATE featureprop SET value = ? " . "WHERE featureprop_id = ?";
    my $sth = $dbh->prepare($query);

    foreach my $tid ( keys %{$data} ) {
        $total_changed++;
        if (   ( $data->{$tid}->{'new'} eq $chp )
            || ( $data->{$tid}->{'new'} eq "hypothetical protein" ) )
        {
            if ( $filter eq "gene_product_name" ) {
                $total_ch_hypo++;
                print $data->{$tid}->{'old'} . "\t=>\tCONSERVED HYPO\n";
                &load_conserved_hypothetical( $tid,
                    $data->{$tid}->{'featureprop_id'},
                    $data->{$tid} );
            }
        } else {
            print $data->{$tid}->{'old'}
...
sub delete_gene_symbol {
    my ($tid) = @_;
    my $sth =
      $dbh->prepare( "SELECT fp.featureprop_id "
    ...
    $sth->execute();
    my $delete_gs =
      $dbh->prepare( "DELETE FROM featureprop " . "WHERE featureprop_id = ?" );
...
sub delete_ec_number {
    ...
    my $delete_fc =
      $dbh->prepare("DELETE FROM feature_cvterm WHERE feature_cvterm_id = ?");
    $sth->execute();
    while ( my $row = $sth->fetchrow_arrayref() ) {
...
sub change_com_name {
    ...
    foreach my $transcript (@transcripts) {
        ...
        unless ( ( $com_name eq $orig )
            && ( $orig ne "hypothetical protein" )
            && ( $orig ne $chp ) )
        {
            $retval{$tid}->{'old'}            = $orig;
...
sub misc_name_changes {
    ...
    if ( $com_name eq
        "transcriptional regulatory protein, C terminal family protein" )
    {
        $com_name = 'putative transcriptional regulator';
    } elsif (
        $com_name eq "bacterial regulatory proteins, luxR family protein" )
    {
        $com_name = "transcriptional regulator, LuxR family";
    } elsif ( $com_name eq
        "bacterial regulatory helix-turn-helix proteins, AraC family protein" )
    {
        $com_name = "transcriptional regulator, AraC family";
    } elsif (
        $com_name eq "bacterial regulatory proteins, tetR family protein" )
    {
        $com_name = "transcriptional regulator, TetR family";
    } elsif (
        $com_name eq "bacterial regulatory proteins, gntR family protein" )
    {
        $com_name = "transcriptional regulator, GntR family";
    } elsif (
        $com_name eq "bacterial regulatory proteins, lacI family protein" )
    {
        $com_name = "transcriptional regulator, LacI family";
    } elsif ( $com_name eq
        "FGGY family of carbohydrate kinases, C-terminal domain protein" )
    {
        $com_name = "carbohydrate kinase, FGGY family";
    } elsif ( $com_name eq
        "tripartite ATP-independent periplasmic transporters, DctQ component family protein"
      )
    {
        $com_name =
          "tripartite ATP-independent periplasmic transporter, DctQ family";
    } elsif ( $com_name eq
        "thiamin/thiamin pyrophosphate ABC transporter, thiamin/thiamin pyrophospate-binding protein"
      )
    {
        $com_name =
          "thiamin/thiamine pyrophosphate ABC transporter, thiamin/thiamine pyrophospate-binding protein";
        ...
        "hydro-lases, Fe-S type, tartrate/fumarate subfamily, beta region domain protein"
      )
    {
        $com_name = "fumarate hydratase family protein";
    } elsif ( $com_name eq
        "glycogen/starch synthases, ADP-glucose type family protein" )
    {
        $com_name = "glycogen/starch synthase";
    } elsif ( $com_name eq
        "glutamate synthases, NADH/NADPH, small subunit domain protein" )
    {
        ...
        "domain related to MnhB subunit of Na+/H+ antiporter family protein" )
    {
        $com_name = "Na+/H+ antiporter family protein";
    } elsif (
        $com_name eq "arginine-tRNA-transferase, C terminus family protein" )
    {
        ...
        $com_name eq "SULFATE TRANSPORTER SULFATE TRANSPORTER FAMILY PROTEIN" )
    {
        ...
        "conserved protein with nucleoside triphosphate hydrolase domain" )
    {
        ...
        $com_name eq "bordetella uptake gene (bug) product family protein" )
    {
        $com_name = "bug family protein";
    } elsif (
        $com_name eq "phage/plasmid replication , gene II/X family protein" )
    {
        $com_name = "phage/plasmid replication protein, gene II/X family";
    } elsif ( $com_name eq
        "invasion gene expression up-regulator, SirB family protein" )
    {
    ...
    } elsif ( ( $com_name eq "PIII" )
        || ( $com_name eq "zn-dependent hydrolase of the beta-lactamase fold" )
      )
    {
        $com_name = $chp;
    }
...
sub contains_underscore {
    ...
    if ( $com_name =~ /_/ ) {
        if ( $com_name eq
            'menC_gamma/gm+: o-succinylbenzoic acid (OSB) synthetase' )
        {
...
sub remove_parens {
    ...
    if ( $com_name =~
        /^(.*([\)\]]))\s*(domain protein|family protein|protein)?$/ )
    {
        my ( $name, $delim, $p ) = ( $1, $2, $3 );
        my $reversed_name = reverse($name);
        $reversed_name =~ tr/\[\]\(\)/\]\[\)\(/;
        my ( $extracted, $remainder ) =
          extract_bracketed( $reversed_name, $delim );
        $remainder =~ tr/\[\]\(\)/\]\[\)\(/;
        $remainder =~ s/^\s+//;
...
sub check_options {
    ...
    {
        open PASS, $options{'password_file'}
          or die( "Cannot open password file "
              . $options{'password_file'}
              . " : $!\n" );
        print STDERR (
            "Password from file will take priority over --password option\n")
        ...
        die(
            "Neither a password or a password file were supplied.  Please supply one or the other"
        );
    }

    open MAP, $options{'input_map_file'}
      or die("Cannot open input mapping file for reading: $!\n");
    my $line = <MAP>;
    chomp $line;
    my ( $unused, $rest );
