...
&GetOptions(
    $options,           "db_mapping_file|d=s",
    "discrep_file|f=s", "user|u=s",
    "password|p=s",     "password_file|P=s",
    "server|s=s",       "delete_url|U=s",
    "no_deletes!",      "log|l=s",
    "help|h",           "man|m",
) || pod2usage();
...
## main program
my ( $dfile, $server, $user, $no_deletes, $delete_url ) = map { $options->{$_} }
  ( 'discrep_file', 'server', 'user', 'no_deletes', 'delete_url' );
...
my $dbh = DBI->connect( $dbi_str, $user, $password,
...
foreach my $gene (@$genes) {
    my ( $cds, $rna, $name, $lines, $hyp ) =
      map { $gene->{$_} } ( 'cds', 'rna', 'name', 'lines', 'hypo' );
...
foreach my $gene (@$genes_to_delete) {
    my ( $cds, $rna, $lines, $transcript_id ) =
      map { $gene->{$_} } ( 'cds', 'rna', 'lines', 'transcript_id' );
    $logger->debug(
        "deleting $cds / $transcript_id, which overlaps with RNA $rna");
    if ( !$no_deletes ) {
        $num_deleted +=
          &delete_gene( $delete_url, $user, $password, $transcript_id, $db );
    }
}
$logger->info(
    "deleted $num_deleted/$num_to_delete gene(s) from $db that have bad CDS features"
);
...
foreach my $rnas (@$rnas_to_delete) {
    my ( $cds, $rna, $lines, $transcript_id ) =
      map { $rnas->{$_} } ( 'cds', 'rna', 'lines', 'transcript_id' );
    $logger->debug(
        "deleting $rna / $transcript_id, which overlaps with CDS $cds");
    if ( !$no_deletes ) {
        $num_deleted +=
          &delete_gene( $delete_url, $user, $password, $transcript_id, $db );
    }
...
$logger->info(
    "deleted $num_deleted/$num_to_delete gene(s) from $db that have bad RNA features"
);
my $rna_delete = $num_to_delete;
my $total      = $rna_delete + $cds_delete;

$logger->warn( "number of deleted genes ("
      . $total
      . ") does not match number of expected unique genes to delete ($ng)" )
  if ( $total ne $ng );
...
sub check_parameters {
    ...
    if (
        defined(
            $options->{'password_file'} && -s $options->{'password_file'}
        )
      )
    {
...
sub read_genes_to_delete {
    ...
    my $pattern =
      [ 'FATAL\:\sDiscRep_SUB:RNA_CDS_OVERLAP', 'DiscRep_SUB:RNA_CDS_OVERLAP' ];
    ...
    while ( my $line = <$fh> ) {
        ...
        if ( ( $line =~ /^$pattern->[0]/ ) || ( $line =~ /^$pattern->[1]/ ) ) {
            ...
            if ( $line =~ /^\s*$/ ) {
                ...
                die
                  "read $num_found instead of the expected $num_expected problem genes at line $lnum of $dfile"
                  if ( ( $num_found + $num_skipped ) != $num_expected );
...
# Do not add genes to array unless they are conserved hypothetical proteins or hypothetical proteins
                    if (   ( $name eq 'conserved hypothetical protein' )
                        || ( $name eq 'hypothetical protein' )
                        || ( $name =~ /^putative/ ) )
                    {
                        $hypo = 1;
                    }
                    die "failed to parse CDS id from $line"
                      unless defined($locus);
                    $cds = $locus;
                } else {
                    $logger->logdie(
                        "failed to parse CDS id from first line of CDS/rRNA pair at line "
                          . ( $lnum - 1 )
                          . ": $line" );
                }
                if ( $next_line =~ /^[^\:]+\:\SRNA.*/ ) {
                    my ( $type, $name, $coords, $locus ) =
                      split( /\t/, $next_line );
                    die "failed to parse RNA id from $next_line"
                      unless defined($locus);
                    $rna      = $locus;
                    $rna_name = $name;
                } else {
                    $logger->logdie(
                        "failed to parse RNA id from second line of CDS/rRNA pair at line $lnum: $next_line"
                    );
                }
                $logger->logdie(
                    "discrepancy file reports CDS $cds overlapping with RNA $rna with the same id"
                ) if ( $cds eq $rna );
                if ( defined( $cds_ids->{$cds} ) ) {
