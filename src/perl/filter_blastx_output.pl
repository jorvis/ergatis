...
open( OFH, ">$$options{'output_file'}" ) or die "Error in writing to the file, $$options{'output_file'}, $!\n";
	...
	while( my $line = <FH> ) {
		...
		if ( $identity >= $identity_cutoff && ( $coverage >= $coverage_cutoff * $$reads_length{ $sub_id } / 100 ) ) {
			unless( exists $$filter{ $query_id } ) {
				push @{ $$filter{ $query_id } }, $sub_start.", ".$sub_end;
				print OFH join( "\t", ( $query_id, $sub_id, $identity, $coverage, $$reads_length{ $sub_id }, $sub_start, $sub_end, $$reads_source{ $sub_id } ) ), "\n";
...
sub is_valid {
	my ( $sub_start, $sub_end, $ref_array ) = @_;
	foreach my $pair ( @$ref_array ) {
		my ( $start, $end ) = split( ", ", $pair );
		if ( ( $sub_start >= $start && $sub_start <= $end ) ||
		     ( $sub_end >= $start && $sub_end <= $end ) || ( $sub_start <= $start && $sub_end >= $end ) ) {
			return 0;
		}
	}
	return 1;
