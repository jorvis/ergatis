...
################
GetOptions(
    \%hCmdLineArgs, 'input_file|i=s', 'username|u=s', 'password|p=s',
    'host|s=s',     'output_dir|o=s', 'tbl_file|t=s', 'log|l=s',
    'help|h'
) or pod2usage();
...
if (   defined( $hCmdLineArgs{'tbl_file'} )
    ...
    && !( defined( $hCmdLineArgs{'username'} ) ) )
{
    fixGeneSymbolsInTbl( $hCmdLineArgs{'tbl_file'},
        $aMeta[3], $hCmdLineArgs{'output_dir'} );
} elsif ( defined( $hCmdLineArgs{'host'} )
    && defined( $hCmdLineArgs{'username'} )
    && defined( $hCmdLineArgs{'password'} )
    && !( defined( $hCmdLineArgs{'tbl_file'} ) ) )
{
    fixGeneSymbolsInDb( \%hCmdLineArgs, \@aMeta );
} else {
    printLogMsg( $ERROR,
        "ERROR : Either a valid .tbl file needs to provided or existing database parameters are needed to fix the gene symbols in .tbl file or database respectively."
    );
}
...
sub fixGeneSymbolsInDb {
    ...
    foreach $sOption (@aRequired) {
        if ( !defined( $phCmdLineArgs->{$sOption} ) ) {
            printLogMsg( $ERROR,
                "ERROR : $sSubName :: Required option $sOption not passed for changing the database $paMeta->[0]"
            );
        }
    }
    ...
    if ( defined($sRFile) && ( -e $sRFile ) && ( -s $sRFile ) ) {
        ...
        printLogMsg( $WARN,
            "WARNING : $sSubName :: Gene symbols rules does not exist or is empty. Thus only duplicate and upper case gene symbols will be removed from database $paMeta->[0]"
        );
    }
    ...
    $pDbh = connectDb(
        $paMeta->[0], "mysql",
        $phCmdLineArgs->{'host'},
        $phCmdLineArgs->{'username'},
        $phCmdLineArgs->{'password'}
    );

# Fetch all the existing gene symbols in the DB and count them to find duplicates
    $sQuery =
      qq(SELECT fp.value FROM featureprop fp, cvterm c WHERE c.name = "gene_symbol" AND c.cvterm_id = fp.type_id AND fp.value!="");
    $pGsyms = $pDbh->prepare($sQuery)
      or printLogMsg( $ERROR,
        "ERROR : $sSubName :: Unable to prepare query : $sQuery. Reason : $DBI::errstr"
      );
    $pGsyms->execute()
      or printLogMsg( $ERROR,
        "ERROR : $sSubName :: Unable to execute query : $sQuery. Reason : $DBI::errstr"
      );
...
# Remove all duplicate and upper case gene symbols and if rules file is provided then replce them too.
    foreach $sGeneSymb ( keys %hRules ) {
        ...
        if ( length($sGeneSymb) == 0 || $sGeneSymb eq "" ) {
            $sQuery =
              qq(DELETE FROM featureprop fp, cvterm c WHERE c.name = "gene_symbol" AND c.cvterm_id = fp.type_id AND (fp.value="$sGeneSymb" OR fp.value=""));
        } else {
            $sQuery =
              qq(UPDATE featureprop fp, cvterm c SET fp.value = "$hRules{$sGeneSymb}" WHERE c.name = "gene_symbol" AND c.cvterm_id = fp.type_id AND fp.value = "$sGeneSymb");
        }
        $pGsyms = $pDbh->prepare($sQuery)
          or printLogMsg( $ERROR,
            "ERROR : $sSubName :: Unable to prepare query : $sQuery. Reason : $DBI::errstr"
    ...
    printLogMsg( $DEBUG,
        "INFO : $sSubName :: Total number of gene symbols changed or deleted in DB $paMeta->[0] is : $nCnt"
    );
    $pDbh->disconnect;
}
...
sub connectDb {
    ...
    if ( $sDbtype eq "mysql" ) {
        ...
        printLogMsg( $ERROR,
            "ERROR : $sSubName :: $sDbtype is not currently a supported database type for the script"
        );
    }
    $pRet =
      DBI->connect( $sDsn, $sUser, $sPasswd,
        { PrintError => 1, RaiseError => 1 } )
      or printLogMsg(
        $ERROR,
        "ERROR : $sSubName :: Could not connect to $sDsn.\nReason: $DBI::errstr"
      );
    return ($pRet);
}
