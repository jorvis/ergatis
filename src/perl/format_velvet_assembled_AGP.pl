...
my (
    $ctbl,     $corrected_tbl, $cfsa,
    $agp,      $num_of_n,      $tbl_base,
    $agp_file, $corrected_fsa, $min_contig_len
);
my ( @old_tbl, @old_fsa );
my $no_table = 0;
my ( %options, %contig_hash );
my $results =
  GetOptions( \%options, "tbl_file|t=s", "fsa_file|f=s", "split_param|n=i",
    "output_dir|o=s", "min_contig_len|m=i", "log|l=s", "help|h" )
  || pod2usage();
...
if ( ( -e $corrected_fsa ) && ( -s $corrected_fsa ) ) {
    ...
    print STDERR
      "Not able to clean parse newly created fasta file $corrected_fsa\n";
}
...
unless ($no_table) {
    ...
    if ( ( -e $corrected_tbl ) && ( -s $corrected_tbl ) ) {
        ...
        system(
            "perl $Bin/format_velvet_assembled_AGP_cleanup.pl -t $corrected_tbl -o $ctbl_cleaned"
        );
...
sub check_options {
    ...
    if ( $options{'tbl_file'} ) {
        my ( $tbl_dir, $tbl_ext );
        ( $tbl_base, $tbl_dir, $tbl_ext ) =
          fileparse( $options{'tbl_file'}, qr/\.[^.]*/ );
        $corrected_tbl =
          $options{'output_dir'} . "/" . $tbl_base . "_corrected" . $tbl_ext;
        open( $ctbl, "> $corrected_tbl" )
          or die "Could not open $corrected_tbl for writing\n";
    } else {
    ...
    if ( $options{'fsa_file'} ) {
        ...
        ( $fsa_base, $fsa_dir, $fsa_ext ) =
          fileparse( $options{'fsa_file'}, qr/\.[^.]*/ );
        $tbl_base = $fsa_base if $no_table;
        $corrected_fsa =
          $options{'output_dir'} . "/" . $fsa_base . "_corrected" . $fsa_ext;
        open( $cfsa, "> $corrected_fsa" )
          or die "Could not open $corrected_fsa for writing\n";
    } else {
...
sub edit_tbl_file {
    ...
    my %contig_genes;
    open( FH, "< $tbl_file" )
      or die "Could not open $tbl_file file for reading. $!\n";
    while (<FH>) {
        $tbl = $_;
        chomp($tbl);
        next if ( $tbl =~ /^\s*$/ );
...
sub process_contig {
    ...
    foreach $count ( sort { $a <=> $b } keys %{$split_hash} ) {
        ...
        foreach $split_head ( keys %{ $contig_hash{$head} } ) {
            ...
            if (
                (
                    $temp_start <= $contig_hash{$head}{$split_head}->{'end_pos'}
                )
                && ( $contig_hash{$head}{$split_head}->{'start_pos'} <=
                    $temp_stop )
              )
            {
    ...
    for ( my $i = 1; $i <= $contig_hash{$head}{'split_num'}; $i++ ) {
        ...
        for ( my $j = 0; $j < @gene; $j++ ) {
            ...
            $start =
              $gene_start - $contig_hash{$head}{$split_head}->{'start_pos'} + 1;
            $stop =
              $gene_stop - $contig_hash{$head}{$split_head}->{'start_pos'} + 1;
            ## Modification made by Sonia Agrawal on 2012/05/25 to split a gene when it falls in two split contigs
            if ( $gene_start < $gene_stop ) {
                if (
                    $contig_hash{$head}{$split_head}->{'end_pos'} < $gene_stop )
                {
                ...
                if ( $contig_hash{$head}{$split_head}->{'end_pos'} <
                    $gene_start )
                {
                    $partial_start = "<";
...
# Convert ASCII code to capital alphabetic character for genes that are split across contigs
            if ( @{ $gene_counter{$count} } > 1 ) {
                @sorted_gene_counter =
                  sort { $a <=> $b } @{ $gene_counter{$count} }
                  if ( $start < $stop );
                @sorted_gene_counter =
                  sort { $b <=> $a } @{ $gene_counter{$count} }
                  if ( $stop < $start );
                @sorted_gene_counter =
                  map { $head . "_" . $_ } @sorted_gene_counter;
            ...
            $codon_start = $1
              if (
                (
                    defined( $split_hash->{$count}{'CDS'} )
                    && $split_hash->{$count}{'CDS'} =~ m/codon_start:::(\d)/
                )
                || ( defined( $split_hash->{$count}{'tRNA'} )
                    && $split_hash->{$count}{'tRNA'} =~ m/codon_start:::(\d)/ )
                || ( defined( $split_hash->{$count}{'rRNA'} )
                    && $split_hash->{$count}{'rRNA'} =~ m/codon_start:::(\d)/ )
              );
            ...
            $frag_len =
              ( $gene_start -
                  $contig_hash{$head}{$split_head}->{'end_pos'} +
                  1 ) - ( $codon_start - 1 )
            ...
            $gl_flag =
              check_gene_length( $start, $stop, $partial_stop, $codon_start );
            next if ( $gl_flag == 1 );
            print $ctbl $partial_start
              . $start . "\t"
              . $partial_stop
              . $stop . "\t"
              . "gene\n";
            ...
            foreach $feat ( sort keys %{ $split_hash->{$count} } ) {
                ...
                for ( my $i = 0; $i < @tags; $i++ ) {
                    ...
                    next
                      if ( $id eq "codon_start"
                        && @{ $gene_counter{$count} } > 1
                        && $alpha > 0 );
                ...
                print $ctbl "\t\t\tcodon_start\t$codon_start\n"
                  if (
                    (
                           @{ $gene_counter{$count} } > 1
                        && length($partial_start) > 0
                        && ( $alpha > 0 )
                    )
                    || ( length($partial_start) > 0 && $flag != 1 )
                  );
                $attrib = $feat if ( $feat =~ /RNA$/ );
                print $ctbl
                  "\t\t\tnote\t5' end; 3' end is $attrib $locus_tag on contig "
                  . $sorted_gene_counter[-1] . "\n"
                  if ( length($partial_stop) > 0
                    && @{ $gene_counter{$count} } > 1
                    && $sorted_gene_counter[-1] ne $split_head
                    && $sorted_gene_counter[0] eq $split_head );
                print $ctbl
                  "\t\t\tnote\t3' end; 5' end is $attrib $locus_tag on contig $sorted_gene_counter[0]\n"
                  if ( length($partial_start) > 0
                    && @{ $gene_counter{$count} } > 1
                    && $sorted_gene_counter[0] ne $split_head
                    && $sorted_gene_counter[-1] eq $split_head );
            }
        }
    }
        ...
        if ( $line =~ /^>(.*)/ ) {
            unless ($flag) {
                ...
                if ( $annot_flag == 1 ) {
                    ...
                    if ( $frag_size == 1 ) {
                        print $agp "$header\t1\t$seq_len\t1\tW\t" . $header
                          . "_1\t1\t$seq_len\t+\n";
...
sub check_contig_len {
    ...
    if ( $seq_length < $min_contig_len ) {
        $annot_yes = 0;
        print STDERR
          "Removing $head from FASTA file as it is less than $min_contig_len bp in length\n";
    }
    return ($annot_yes);
}
...
## Modifications made by Sonia Agrawal on May 29, 2012 to remove gaps from the beginning and end of a split scaffold in the .agp file and always start the first contig in the split scaffold with 1
        for ( $j = 0; $j < @print_contigs; $j++ ) {
            if (
                (
                    $contig_hash{$head}{ $print_contigs[$j] }{'start_pos'} >
                    $start_pos
                )
                && ( $start_pos != 1 )
              )
            {
                $end_pos =
                  $contig_hash{$head}{ $print_contigs[$j] }{'start_pos'} -
                  $sub_offset - 1;
                print $agp "$head\t$start_pos\t$end_pos\t$part_num\tN\t"
                  . ( $end_pos - $start_pos + 1 )
                  . "\tscaffold\tyes\tpaired-ends\n";
                $part_num++;
            } else {
                $sub_offset =
                  $contig_hash{$head}{ $print_contigs[$j] }{'start_pos'} - 1;
            }
            print $agp "$head\t"
              . ( $contig_hash{$head}{ $print_contigs[$j] }{'start_pos'} -
                  $sub_offset )
              . "\t"
              . ( $contig_hash{$head}{ $print_contigs[$j] }{'end_pos'} -
                  $sub_offset )
              . "\t$part_num\tW\t$print_contigs[$j]\t1\t$contig_hash{$head}{$print_contigs[$j]}{'length'}\t+\n";
            $part_num++;
            $start_pos =
              $contig_hash{$head}{ $print_contigs[$j] }{'end_pos'} -
              $sub_offset + 1;
...
sub read_file {
    ...
    my @lines;
    open( FH, "< $filename" )
      || die("Could not open $filename file for reading.$!");
    @lines = <FH>;
    close(FH);
    return (@lines);
}
