...
gap22legacydb.pl - load gap2 BSML file into the legacy database schema

=head1 SYNOPSIS

USAGE:  gap22legacydb.pl
...
B<--bsml_file,-b> 
    Path to a single BSML gap2 result file.

B<--list_file,-i> 
    Path to a list of gap2 BSML output files.

B<--database,-d> 
    Sybase database name for the project in which these gap2 predictions
...
This script is used to load gene predictions from the gap2 program, in BSML format,
into one of the legacy databases.  Such BSML files are most likely generated from
the gap2 workflow component.
...
my $qry = "INSERT INTO evidence (feat_name, ev_type, accession, end5, end3, rel_end5, rel_end3, m_lend, m_rend, curated, date, assignby, change_log, save_history, method, per_id, per_sim, score, db, pvalue, domain_score, expect_domain, total_score, expect_whole, chainID) " .
          "VALUES (?, 'gap2', ?, ?, ?, -1, -1, ?, ?, 0, getdate(), 'workflow', 0, 0, 'aat_na', ?, ?, ?, ?, NULL, ?, NULL, ?, NULL, ?)";
