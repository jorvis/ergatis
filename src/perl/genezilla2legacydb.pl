...
genezilla2legacydb.pl - load genezilla BSML file into the legacy database schema

=head1 SYNOPSIS

USAGE:  genezilla2legacydb.pl
...
B<--bsml_file,-b> 
    Path to a single BSML genezilla result file.

B<--list_file,-i> 
    Path to a list of genezilla BSML output files.

B<--database,-d> 
    Sybase database name for the project in which these genezilla predictions
    will be loaded.
...
This script is used to load gene predictions from the genezilla program, in BSML format,
into one of the legacy databases.  Such BSML files are most likely generated from
the genezilla workflow component.

=head1 INPUT

The input is a BSML result file.  See the genezilla component documentation for the
...
be in the format like:
    ...
    aa1.assembly.16296.0.genezilla.bsml
...
for (@files) {
    ...
    ##  aa1.assembly.(15588).0.genezilla.bsml
    my $asmbl_id;
    if ($file =~ /\.assembly\.(\d+)/) {
        $asmbl_id = $1;
    } else {
    ...
    my $loader = new Annot_prediction_loader($dbh, $asmbl_id, 'GeneZilla');
    $logger->debug("loading these predictions for file $file:\n" . Dumper(\@genes) . "\n") if ($logger->is_debug);
...
sub processFeatureGroup {
    ...
    ## genezilla sometimes calls a transcript group that doesn't contain an
    ##  exon.  it may be composed solely of a poly-A-signal, for example.  We need
    ##  to warn and skip these.
    if (scalar keys %exon_coords) {
        ...
        $logger->warn("skipping feature group " . $fg->{att}->{'group-set'} . " because it had no exons") if ($logger->is_warn);
    }
}
