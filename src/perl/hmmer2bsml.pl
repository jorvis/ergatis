...
USAGE: hmmer2bsml.pl 
      ...
      [ --search_method=hmmpfam or hmmscan
        --query_mol_type=aa
...
appropriate titles in the BSML Analysis element.  If not passed, the default 'hmmscan' is used.
...
} elsif ($options{'search_method'} eq 'hmmscan') {
...
## handle defaults
	$options{'search_method'} = 'hmmscan' unless ( $options{'search_method'} );
