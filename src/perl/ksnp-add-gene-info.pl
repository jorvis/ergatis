...
my %query_fsa_files;
%query_fsa_files = &map_fasta_inputs( $options{'query_fasta'} );
...
sub process_overlap {
	...
	#Removing for kSNP since output does not provide this information
	...
	my $refcodon = &get_sequence_region( $reference_genes{$gene_id}->{'seq'},
										 ...
										 1 );
	# Removing below since kSNP does not provide this output
=forcomment
...
sub check_options {
   ...
   foreach my $req ( qw(reference_genbank show_snps_file output query_fasta) ) {
