...
make_gene_pep_fasta.pl  - Generates protein sequences in FASTA format from BSML files
...
make_gene_pep_fasta.pl is designed to generate multi-fasta files of protein sequences
...
by --name flag.  By default, the fasta header is in the form ">protein_id".
If --allvsall_header is invoked the resulting header will be ">asmbl_id:protein_id".  This
was designed for pallvsall. Mode 2 generates one fasta file per assembly_ID.  The file
will be named "assembly_id.pep" i.e. (bsp_3839_assembly.pep).  Mode 3 generates one fasta
file per gene.  The fasta files will be stored in the respective asmbl_id directory.
...
1. make 1 fasta file called PNEUMO.pep containing sequences from assembly A and B   
   make_gene_pep_fasta.pl -b bsml_dir -o output_dir -a A,B -name PNEUMO

2. make N fasta files for N assembly IDs stored in a file "list.txt"
   make_gene_pep_fasta.pl -b bsml_dir -o output_dir --asmbl_file list.txt --mode 2
...
umask(0000);
my %options = ();
my $results = GetOptions (\%options, 'bsml_dir|b=s', 'asmbl_ids|a=s', 'allvsall_header|s', 'name|n=s', 'verbose|v', 'log|l=s', 'debug|D=s',
...
my $allvsall_header = $options{'allvsall_header'};
...
if($mode == 1) {
    make_total_fasta(\@asm_ids, $fasta_name);
}elsif($mode == 2) {
    make_fasta_per_genome(\@asm_ids);
}elsif($mode == 3) {
...
sub make_total_fasta {
#this mode outputs 1 protein fasta file for ALL assembly IDs

    my $assembly_ids = shift;
    my $fasta_name   = shift;

    my $pep_file = "$output_dir/$fasta_name.pep";
    open(FASTA, ">$pep_file") || die "Cant open $pep_file due to $!";
	...
	if (-s $bsml_file) {
		...
		$identifier =~ s/_\d$//;
		my $prot_name = $allvsall_header ? "${asmbl_id}:$identifier" :  "$identifier";
    ...
    close FASTA;
    unlink($pep_file) if (-z $pep_file);  #delete empty file
    chmod 0777, $pep_file;

}


sub make_fasta_per_genome {
#this mode outputs 1 protein fasta file per assembly ID
	...
	open(FASTA, ">$pep_file") || die "Unable to write to $pep_file due to $!";
	my $bsml_file = "$BSML_dir/${asmbl_id}.bsml";
	if (-s $bsml_file) {
	    my $reader = BSML::BsmlReader->new();
	    $parser->parse( \$reader, $bsml_file );
	    ...
	    print STDERR "$bsml_file NOT found, skipping...\n";
	    unlink($pep_file) if (-e $pep_file);  #delete empty file
	    #$logger->logdie("$bsml_file NOT found!  Exiting...");
	}
    }
...
sub make_fasta_for_each_gene  {
	...
	if (-s $bsml_file) {
	    ...
	    print STDERR "$bsml_file NOT found, skipping...\n";
	    rmdir($final_output_dir);  #delete empty file
	    #$logger->logdie("$bsml_file NOT found!  Exiting...");
	}
