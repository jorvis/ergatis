...
sub make_fasta_for_each_gene  {
	...
	next if($asmbl_id !~ /\d+/);
	my $final_output_dir = "$output_dir/asmbl_id_".$asmbl_id;
...
sub make_fasta_for_each_genome {
    ...
    foreach my $asmbl_id (@$assembly_ids) {
	my $pep_file = "$output_dir/asmbl_id_".$asmbl_id.".pep";
	open(FILE, ">$pep_file") || die "Unable to write to $pep_file due to $!";
	my $bsml_file = "$BSML_dir/asmbl_${asmbl_id}.bsml";
	if (-s $bsml_file) {
	    my $reader = BsmlReader->new();
...
sub make_PNEUMO_pep_for_ALL_genomes {
	...
	if (-s $bsml_file) {
	    ...
	    while (my ($identifier, $seq) = each %$proteins) {
		$identifier =~ s/_\d$//;
		my $prot_name = $simple_header ? "$identifier asmbl_id_${asmbl_id}" : "asmbl_id_${asmbl_id}:$identifier";
		my $fastaout = &fasta_out($prot_name, $seq);
                print FILE $fastaout;
            }
	} else {
