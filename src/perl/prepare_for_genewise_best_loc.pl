...
strand contains the best hits per location.  
...
my ($database, $username, $password, $search_db, $work_dir,
    $asmbl_id, $asmbl_file, $file_list, $help, $verbose);
...
open (my $file_list_fh, ">$file_list") or croak ("cannot write to $file_list ");
...
sub get_best_location_hits {
	...
	if ($chain_struct->{orient} eq '+') {
	    &try_add_chain($chain_struct, \@top_strand_feats);
	} else {
	    &try_add_chain($chain_struct, \@bottom_strand_feats);
	}
...
####
sub try_add_chain {
    my ($chain_struct, $feature_list_aref) = @_;
