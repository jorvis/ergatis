...
Generate BER search data from a BSML data repository. Each input sequence is searched against all sequences in the repository.

USAGE: run_ber -d DATABASE_NAME 
...
my %options = ();
my $results = GetOptions (\%options, 'asmbl_file|f=s', 'database|d=s', 'config|c=s', 'printconf|p', 'help|p' );
...
# The default ber configuration

my $conf = {
    ';TMP_DIR;' => '/usr/local/scratch',
    ';REPOSITORY_ROOT;' => '/usr/local/annotation',
    ';MASTER_CONF;' => "$WorkflowConfigFileDir/ber-master.ini",
    ';MASTER_TEMPLATE;' => "$WorkflowConfigFileDir/ber-master_template.xml",
    ';SUBFLOW_CONF;' => "$WorkflowConfigFileDir/ber.ini", 
    ';SUBFLOW_TEMPLATE;' => "$WorkflowConfigFileDir/ber_template.xml",
...
system "cp $conf->{';SUBFLOW_TEMPLATE;'} $conf->{';REPOSITORY_ROOT;'}/$conf->{';DATABASE;'}/Workflow/$conf->{';WFNAME;'}/$conf->{';WFID;'}/ber_template.xml";
system "chmod 666 $conf->{';REPOSITORY_ROOT;'}/$conf->{';DATABASE;'}/Workflow/$conf->{';WFNAME;'}/$conf->{';WFID;'}/ber_template.xml";
