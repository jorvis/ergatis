...
Generate blastp search data from a BSML data repository. Each input sequence is searched against all sequences in the repository.

USAGE: run_blastp -d DATABASE_NAME 
...
=cut
#pblastp documentation at http://wild/ifx/devel/HTC/psearch_info/doc/PSEARCH_User_Guide.pdf
#http://blast.wustl.edu/
#pblastp_echofilter=default
#pblastp_expect=default
#pblastp_postsw=default
#pblastp_preargs=default
#pblastp_matrix=default
#pblastp_wordlength=default
#pblastp_nogap=default
#pblastp_descriptions=default
#pblastp_filter=default
#pblastp_report=default
#pblastp_alignments=default
#pblastp_cutoff=default
#btab2bsml_pvalue_cutoff=1e-15

my %options = ();
my $results = GetOptions (\%options, 'asmbl_file|f=s', 'database|d=s', 'config|c=s', 'printconf|p', 'help|h' );
...
# The default blastp configuration
...
my $conf = {
    ...
    ';MASTER_CONF;' => "$WorkflowConfigFileDir/blastp-master.ini",
    ';MASTER_TEMPLATE;' => "$WorkflowConfigFileDir/blastp-master_template.xml",
    ';SUBFLOW_CONF;' => "$WorkflowConfigFileDir/blastp.ini", 
    ';SUBFLOW_TEMPLATE;' => "$WorkflowConfigFileDir/blastp_template.xml",
...
system "cp $conf->{';SUBFLOW_TEMPLATE;'} $conf->{';REPOSITORY_ROOT;'}/$conf->{';DATABASE;'}/Workflow/$conf->{';WFNAME;'}/$conf->{';WFID;'}/blastp_template.xml";
system "chmod 666 $conf->{';REPOSITORY_ROOT;'}/$conf->{';DATABASE;'}/Workflow/$conf->{';WFNAME;'}/$conf->{';WFID;'}/blastp_template.xml";
