#!/usr/local/bin/perl
#---------------------------------------------------------------------
# script name: run_bsmlscaffold2chado.pl
# date:        2003.10.28
#
#
#
...
run_bsmlscaffold2chado.pl - Creates and executes the bsml to chado migration workflow

=head1 SYNOPSIS

USAGE:  run_bsmlscaffold2chado.pl -U username -P password -D target_database [-t target_server] [-f organism_file|-o organism_list] [-b bsml_type] [-f bsml_batch_file] [-c config] [-h] [-m] [-p] [-q] [-v]
...
#
# The default bsmlscaffold2chado configuration
#
my $conf = {
    ';INSTALLATION;'                         => $WorkflowExecDir,
    ';TMP_DIR;'                              => '/usr/local/scratch/bsmlscaffold2chado',
    ';REPOSITORY_ROOT;'                      => '/usr/local/annotation',
    ';WORKFLOW_DIR;'                         => $WorkflowExecDir,
    ';SET_RUNTIME;'                          => $WorkflowExecDir ."/set_runtime_vars.pl",
    ';BSMLSCAFFOLD2CHADO_CONF;'              => $WorkflowConfigFileDir ."/bsmlscaffold2chado.ini", 
    ';BSMLSCAFFOLD2CHADO_MASTER_CONF;'       => $WorkflowConfigFileDir ."/bsmlscaffold2chado-master.ini", 
    ';BSMLSCAFFOLD2CHADO_TEMPLATE;'          => $WorkflowConfigFileDir ."/bsmlscaffold2chado_template.xml",
    ';BSMLSCAFFOLD2CHADO_MASTER_TEMPLATE;'   => $WorkflowConfigFileDir ."/bsmlscaffold2chado-master_template.xml",
    ';RUN_WF;'                               => $WorkflowExecDir ."/run_wf.sh",
    ';WFNAME;'                               => 'bsmlscaffold2chado',	
    ';USERNAME;'                             => '',
    ';PASSWORD;'                             => '',               
    ';TARGET_DATABASE;'                      => '',               
...
my $log4perl = "bsmlscaffold2chado_wf.log";
...
#
# Store the master configuration for the bsmlscaffold2chado workflow instances to follow
...
#
# Perform the substitution on the master bsmlscaffold2chado workflow
# and set global permissions
#
$execution_string = $set_runtime ." -c ". $master_wf_fullpath ." < ". $conf->{';BSMLSCAFFOLD2CHADO_MASTER_CONF;'} ." > ". $workflow_instance_dir ."/bsmlscaffold2chado-master-instance.ini";
&execute(\$execution_string);
...
foreach my $bsml_item (sort keys %$bsml_hash){
    ...
    #
    $conf->{';BSMLSCAFFOLD2CHADO_LOGFILE;'}    = $workflow_instance_dir ."/". $bsml_basename ."/bsmlscaffold2chado.log";
    ...
    #
    $execution_string = $set_runtime ." -c ". $bsml_subflow ." < ". $conf->{';BSMLSCAFFOLD2CHADO_CONF;'} ." > ". $workflow_instance_dir ."/". $bsml_basename .".ini";
    &execute(\$execution_string);

}

#
# Copy the bsmlscaffold2chado_template.xml        to bsmlscaffold2chado-instance_template.xml AND
#          bsmlscaffold2chado-master_template.xml to bsmlscaffold2chado-master-instance_template.xml
#
# and then set global permissions
#

$execution_string = "cp ". $conf->{';BSMLSCAFFOLD2CHADO_TEMPLATE;'} ." ". $workflow_instance_dir ."/bsmlscaffold2chado-instance_template.xml";
&execute(\$execution_string);
$execution_string = "cp ". $conf->{';BSMLSCAFFOLD2CHADO_MASTER_TEMPLATE;'} ." ". $workflow_instance_dir ."/bsmlscaffold2chado-master-instance_template.xml";
&execute(\$execution_string);

#
# Run the bsmlscaffold2chado workflow
# The run_wf script currently executes the following steps:
# 1) CreateWorkflow
# 2) RunWorkflow
#
$execution_string = $run_wf ." -d ". $workflow_instance_dir ." -c ". $workflow_instance_dir ."/bsmlscaffold2chado-master-instance.ini" . " -t ". $workflow_instance_dir ."/bsmlscaffold2chado-master-instance_template.xml" . " -i ". $workflow_instance_dir ."/bsmlscaffold2chado.xml" ." -l ". $workflow_instance_dir ."/log.txt";
&execute(\$execution_string);


#
# Move the bsmlscaffold2chado log4perl log file to the workflow instance directory
#
...
sub sample_bsml_batch_file {
    ...
    print STDERR "a)\t1=scaffold and assembly mapping\n";
...
sub print_usage {
    ...
    print STDERR "  -b|--bsml_type         = 1=scaffold and assembly mapping\n";
