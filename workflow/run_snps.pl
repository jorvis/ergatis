...
Generate SNPS search data from a BSML data repository. 

USAGE: run_snps -d DATABASE_NAME -r REF_ASSEMBLY_ID -q QUERY_ASSEMBLY_FILE 
...
B<--ref_asmbl, -r> [REQUIRED] the reference assembly id

B<--query_file, -q> [OPTIONAL]  a file containing the assembly ids corresponding to the query sequences, one per line
...
# The default snps configuration
...
if( $options{'ref_asmbl'} )
{
    $conf->{';REF_ASMBL;'} = $options{'ref_asmbl'};
}

if( $options{'query_file'} )
{
    $conf->{';QUERY_ASMBL_FILE;'} = $options{'query_file'};
}
...
# Verify that a reference assembly has been specified

if( !( $conf->{';REF_ASMBL;'} ))
{
    print STDERR "ERROR: A Reference Assembly was not provided\n";
    exit(1);
}

# Verify that the query assembly file exists

if( !( -e  $conf->{';QUERY_ASMBL_FILE;'} ))
{
    print STDERR "ERROR: A Query Assembly File was not provided\n";
    exit(1);
} 
