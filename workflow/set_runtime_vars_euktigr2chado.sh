...
do case "$opt" in
      ...
      h) echo "Usage: `basename $0` -s database -a asmbl_id -t target_database -v target_server -w wf_name -r run_dir";
         echo;
	 exit 1;;
      esac
done
...
echo "Setting database:$database target_database:$target_database asmbl_id:$asmbl_id for $wfname"
